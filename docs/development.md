...
To develop gltcore-node:

```bash
cd ~
git clone git@github.com:<yourusername>/gltcore-node.git
git clone git@github.com:<yourusername>/gltcore-lib.git
```
...
We now will setup symlinks in `gltcore-node` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf gltcore-lib
ln -s ~/gltcore-lib
...
cd devnode
mkdir node_modules
touch gltcore-node.json
touch package.json
```

Edit `gltcore-node.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../gltcore-node/bin/gltcore-node start
```