...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'gltcore-lib': '^' + bitcore.version,
    'gltcore-node': version
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/gltcore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the gltcore-node directory and configuration
